...
const normalizeVideo = (v) => {
  ...
  if (youtubeId) {
    return {
      ...
      youtubeId,
      type: "youtube",
      src: `https://www.youtube.com/embed/${youtubeId}?playsinline=1&autoplay=1&mute=0`,
      thumbnail:
        v.thumbnail || `https://img.youtube.com/vi/${youtubeId}/hqdefault.jpg`,
    };
  }
...
export default function VideoCard() {
  ...
  // Volume state: default 0.5 (50%)
  const [volume, setVolume] = useState(0.5);
  const [muted, setMuted] = useState(false);

  // Preload state: store the next video's blob URL or src
  ...
  const filteredVideos = useMemo(() => {
    ...
    return videos.filter((v) => {
      const hay =
        `${v.title || ""} ${v.description || ""} ${v.youtubeChannel || ""} ${v.youtubeUrl || ""}`.toLowerCase();
  ...
  // Preload next uploaded video
  useEffect(() => {
    if (!nextVideo) {
      setPreloadedSrc(null);
      return;
    }
    if (nextVideo.type !== "uploaded") {
      // For YouTube, we preconnect and preload the iframe later (optional)
      setPreloadedSrc(null);
      return;
    }

    const src = nextVideo.src;
    if (!src) return;

    // If already preloaded same src, skip
    if (preloadVideoRef.current && preloadVideoRef.current.src === src) return;

    // Use a hidden video element to preload
    ...
    preloadVideoRef.current.src = src;
  ...
  const fetchVideos = useCallback(async () => {
    ...
    try {
      ...
      normalized.forEach((v) => {
        if (myId && Array.isArray(v.likedBy))
          likedMap[v._id] = v.likedBy.some((x) => String(x) === String(myId));
  ...
  // Volume handling for uploaded videos
  useEffect(() => {
    ...
    video.muted = muted;
    video.play().catch(() => {});
  }, [currentVideo?._id, currentVideo?.type, volume, muted]);

  // Set initial volume to 0.5 when video element is ready
  useEffect(() => {
    if (!currentVideo) return;
    if (currentVideo.type !== "uploaded") return;
    if (!videoRef.current) return;
  ...
  const handleLikeToggle = async () => {
    ...
    try {
      ...
      if (data.success && data.video) {
        setVideos((prev) =>
          prev.map((v) =>
            v._id === data.video._id ? normalizeVideo(data.video) : v,
          ),
        );
        setLikedVideos((prev) => ({
          ...prev,
  ...
  const handleShare = async () => {
    ...
    fetch(`${API_ENDPOINT}/${currentVideo._id}/share`, {
      method: "POST",
    }).catch(() => {});
  ...
  const handleDownload = async () => {
    if (currentVideo?.type !== "uploaded")
      return alert("YouTube videos cannot be downloaded directly.");
    if (!currentVideo?.src) return;

    const url = currentVideo.src;
    const filename = (
      url.split("?")[0].split("/").pop() || "video.mp4"
      ...
      <div className="mx-auto max-w-7xl px-4 py-4">
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-4">
          ...
          <div className={cn("lg:col-span-6", isMobile ? "col-span-1" : "")}>
            <div
              className="relative rounded-2xl border border-white/10 bg-black overflow-hidden"
            ...
            >
              <div
                ...
                onClick={
                  currentVideo.type === "uploaded" ? handleDoubleTap : undefined
                }
              >
                {currentVideo.type === "uploaded" ? (
                  <video
                    ...
                    playsInline
                    onVolumeChange={(e) => {
                      if (!muted) setVolume(e.target.volume);
                    }}
                  />
                ) : currentVideo.type === "youtube" ? (
                  <iframe
                  ...
                  <div className="w-full h-full flex items-center justify-center text-white/70 p-6 text-center">
                    This item has no playable source. Check your backend
                    response fields.
                  </div>
                )}
                ...
                <div className="absolute right-3 top-1/2 -translate-y-1/2 flex flex-col gap-3">
                  ...
                  >
                    <Heart
                      size={20}
                      className={cn(isLiked ? "text-red-500" : "text-white")}
                    />
...
function CommentsPanel({
  ...
  return (
    <div
      className={cn(
        "flex flex-col",
        compact ? "h-[75vh]" : "h-[calc(100vh-200px)]",
      )}
    >
                    ...
                    {showReplyInput[comment._id] && (
                      <div className="mt-3 flex gap-2">
                        <input
                          ...
                          placeholder={
                            user ? "Write a reply…" : "Login to reply…"
                          }
                          disabled={!user}
                        ...
                        <button
                          ...
                          disabled={
                            !user || !(replyTexts[comment._id] || "").trim()
                          }
