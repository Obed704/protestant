...
const VideoCard = () => {
  ...
  const fetchVideos = async () => {
    ...
    try {
      ...
      if (data.success) {
        ...
        const fav = {};
        list.forEach((v) => {
          liked[v._id] = !!v.likedByMe;
          fav[v._id] = !!v.favoritedByMe;
        });
        setLikedVideos(liked);
        setFavoritedVideos(fav);
  ...
  useEffect(() => {
    if (currentVideo?._id) {
      fetch(`${API_ENDPOINT}/${currentVideo._id}/view`, { method: "POST" }).catch(console.error);
    }
  }, [currentVideo?._id]);

  const patchVideoInList = (updatedVideo) => {
    const updatedVideos = [...videos];
    updatedVideos[currentIndex] = updatedVideo;
    setVideos(updatedVideos);

    setLikedVideos((prev) => ({ ...prev, [updatedVideo._id]: !!updatedVideo.likedByMe }));
    setFavoritedVideos((prev) => ({ ...prev, [updatedVideo._id]: !!updatedVideo.favoritedByMe }));
  };
  ...
  const handleLikeToggle = async () => {
    ...
    try {
      ...
      if (data.success) patchVideoInList(data.video);
    } catch (err) {
      console.error(err);
  ...
  const handleFavoriteToggle = async () => {
    ...
    try {
      const res = await fetch(`${API_ENDPOINT}/${currentVideo._id}/favorite`, {
        method: "POST",
        headers: {
          "Content-Type": "application/json",
          ...(token ? { Authorization: `Bearer ${token}` } : {}),
      ...
      console.error(err);
      alert(err.message || "Error favoriting video");
    }
  };

  const handleShare = async () => {
    const shareUrl = currentVideo.youtubeUrl || currentVideo.src;
    ...
    // optional: count shares server-side
    fetch(`${API_ENDPOINT}/${currentVideo._id}/share`, { method: "POST" }).catch(() => {});
  };

  const goNext = () => currentIndex < videos.length - 1 && setCurrentIndex(currentIndex + 1);
      ...
      <div className="relative flex-1 flex items-center justify-center bg-black">
        ...
        <div className="absolute right-4 top-1/2 transform -translate-y-1/2 flex flex-col gap-4 z-10">
          ...
          >
            ...
            <span className="text-xs mt-1">{currentVideo.likesCount}</span>
          </button>

          <button
            onClick={handleFavoriteToggle}
            className={`flex flex-col items-center p-3 rounded-full shadow-lg transition ${
              favoritedVideos[currentVideo._id]
                ? "bg-yellow-500 text-black"
                : "bg-white/20 text-white backdrop-blur-sm"
            }`}
          >
            <span className="text-2xl">⭐</span>
            <span className="text-xs mt-1">{currentVideo.favoritesCount}</span>
          </button>
      ...
      {(showCommentsMobile || !isMobile) && (
        ...
        >
          <div className="p-4 border-b border-gray-800 flex justify-between items-center bg-gray-900">
            ...
            {isMobile && (
              <button className="text-white text-xl" onClick={() => setShowCommentsMobile(false)}>
                ✕
              </button>
            )}
          </div>
